...
def powerpoint2text(file_path):
    text = ""
    ppt = win32com.client.gencache.EnsureDispatch("PowerPoint.Application")
    ppt.Visible = True # アプリが開く(0, Falseにするとエラー on Python2.7, Windows7, Office2003)
    ppt.DisplayAlerts = False # 警告OFF
    try:
        ppt_file = ppt.Presentations.Open(file_path, True) # 読み取り専用で開く
        slide_count = len(ppt_file.Slides) + 1
        for i in range(1, slide_count):
            slides = ppt_file.Slides(i)
            shape_num = slides.Shapes.Count + 1
            for j in range(1, shape_num):
                try:
                    text += slides.Shapes(j).TextFrame.TextRange.Text + " "
                except:
                    pass
            text += '\n'
        ppt_file.Close()
    ...
    return text
def main():
    print powerpoint2text('C:\サンプル\python_win32com.ppt')
